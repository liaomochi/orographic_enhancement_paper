...
wso = ws;
wsmo = wsm;
disp(['ws original mean ' num2str(mean(ws(:)),'%5.5f')])
disp(['wsm original mean ' num2str(mean(wsm(:)),'%5.5f')])
...
fid = fopen('rising_pt.txt','r');% here is very different###########!!!!!!!!!
fom = '%d';
ck = fscanf(fid,fom);
fclose(fid);
...
disp(['=============== rising is point ' num2str(ck)])
...
if modd == 2
    ...
    rdif = (sum(simu(ck:ploc))-sum(obsnow(ck:ploc)))*8*3600;
    ...
    for jh = cdy%ak:-1:cdy  %2024/11/25 changed back to 3500 from 3600
        %
        ...
        if ak>=(rains_bd+1)
            if rains_bd>=2
                wp = wron{ak,rains_bd+1}+wron{ak,rains_bd-1}+wron{ak,rains_bd};%3900 1129evening
            else
                wp = wron{ak,1}+wron{ak,2}+wron{ak,3};
            end
        elseif ak>=(rains_bd-1)
            wp = wron{ak,rains_bd-1}+wron{ak,rains_bd-2}+wron{ak,rains_bd-3};
        else
            wp = wron{ak,ak}+wron{ak,ak-1}+wron{ak,ak-2};
        end

        % 2024/11/25 for wp
        % wp = zeros(Basins_sum(gloc,7),Basins_sum(gloc,6));
        % 
        % if (rains_bd-4)>=ak% rising point is ahead of 'systematic definition of rainfall starting point', slightly adjust ak
        %     ak = rains_bd+5;
        %     for interpo = -4:4
        %         for witr = -4:4
        %             if (rains_bd+witr)<ak
        %                 wp = wp + wron{ak+interpo,rains_bd+witr+interpo};
        %                 disp(['adjusted (ak, ) pairs are' num2str(ak,rains_bd+witr)])
        % 
        %             else
        %                 disp(['adjusted out of boundary (ak, ) pairs are' num2str(ak) ' ' num2str(rains_bd+witr)])
        ...
        if ~isempty(rowi)
            ...
            eligi_pt = find(wp>=1);
            disp(['non empty tracking wp= ' num2str(length(eligi_pt))])
            dis_tan = dis_outlet(eligi_pt);
            dis_str = dis_stream(eligi_pt);
            if ismember(0, dis_str)&&length(eligi_pt)>=3
                distance_thre = max(dis_outlet(eligi_pt(find(dis_str==0))));% 08052024 here takes max instead of min for slow recession
            ...
            disp(' empty tracking')
            dis_str = 0;% experimental 11/25/2024
            distance_thre = 0.5*max(dis_outlet(:));% added 08=03-2024 to avoid the no definition of distance_thre
        end
    end
    ...
    final_corr_area(find(dis_stream<=max(dis_str)&dis_outlet<=distance_thre)) = 1;% 08052024 here <= % 11252024 dis_stream<=max(dis_str) not ==0 anymore
    ...
    disp(['$$ adjust ratio is ' num2str(sumsm)])
...
%end
disp(['ws updated mean ' num2str(mean(ws(:)),'%5.5f')])
disp(['wsm updated mean ' num2str(mean(wsm(:)),'%5.5f')])
